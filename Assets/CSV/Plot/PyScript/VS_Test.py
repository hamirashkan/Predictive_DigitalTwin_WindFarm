...
def forcast(csv_path, value,indx1,indx2,turbineName):
    ...
    merged_data = pd.read_csv(csv_path + "Saved_data - Backup.csv")
    ...
    x = pd.read_csv(csv_path + "Predicted.csv")
    df = pd.DataFrame(x);
    ...
    df.to_csv(csv_path + 'Predicted.csv',index=False)
    ...
    future_data = m.make_future_dataframe(periods = ftr, freq = 'sec')
    ...
    # print('Displaying Prophet plot')
    fig1 = m.plot(forecast)
    fig1 = prophet_faultydata.plot(figsize = (16,6),title=turbineName,linewidth = 1.0, linestyle='-',color='red')
    fig1.set(xlabel="Month (MM)-Date(DD) Time", ylabel="Temperature(C)")
